...
make_ess_table = function (strain) {
  ...
  ess_matrix[[tag_ess]] <- calculate_essentiality(ess_matrix[[tag_ii]])  
  cat(sprintf("==========================================================================\n\n"))
...
calculate_essentiality = function (ii_vector) { 
  library(ggplot2)
  library(crayon)
  ...
  p2 <- ggplot(ii_df,aes(x=ii_vector)) + geom_histogram(color="black",fill="light blue",bins=100) + xlim(-1e-3,5*maxval) + 
  ...
  cat(sprintf("%d essential, %d ambiguous, and %d non-essential genes.\n",
              length(ess_index),length(amb_index),nG-length(ess_index)-length(amb_index)))
  return(calls)
}
